...
.PHONY: all export lint fmt-check fingerprint serve build deploy clean help

all: export lint fmt-check fingerprint build
...
fingerprint:
	$(PYTHON) -m minerupress.fingerprint --docs-dir $(DOCS_OUT) --out $(REPORTS)/fingerprints.json
